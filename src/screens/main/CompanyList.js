...
export default function CompanyList(props) {
  ...
  const getAllMemberComapnyList = () => {
    isInternetConnected()
      .then(() => {
        dispatch(getMemberComapnyList("company"));
      })
      .catch(err => {
  ...
  return (
    <>
      ...
      <SafeAreaView style={{flex: 1, backgroundColor: Colors.white}}>
        <Header
          back={true}
          title={translate("All Companies")}
          add={true}
          addPress={() => props.navigation.navigate('AddCompany')}
          {...props}
        />
        <ScrollView>
              ...
              {allCompany &&
                allCompany.map((item, i) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      props.navigation.navigate('AddCompany', {id: item?.id,edit:true});
                    }}>
                    <View style={{flex: 1}}>
                      <Text style={style.itemText} numberOfLines={1}>{item?.name}</Text>
